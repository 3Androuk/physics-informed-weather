...
def patch_latitudes(patch_dir, indices, size: int,
                    split: str = "test") -> np.ndarray:
    """Per-patch latitude rows (len(indices), size) from saved origins + coords.

    `indices` are the test-set patch indices actually evaluated (must match the
    order the fields were stacked in — a strided or shuffled selection weights
    the wrong rows otherwise). An int is accepted as shorthand for range(n).
    ...
    return np.stack([lat_full[int(origins[int(i), 0]):
                              int(origins[int(i), 0]) + size] for i in indices])
